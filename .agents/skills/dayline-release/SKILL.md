...
1. Audit and establish the release baseline before mutation.
   ...
   - Inspect stable tags, open PRs, GitHub releases and drafts, recent CI/release runs, and the active `main` ruleset. Confirm the repository-scoped `APPCAST_DEPLOY_KEY` secret exists so the notarization workflow can publish the signed Sparkle feed through its narrow ruleset bypass; if the secret is absent, stop and report the missing prerequisite instead of continuing. Detect any existing release draft for the same tag and exact commit so it can be resumed instead of duplicated.
...
3. Update the changelog through its own PR before tagging.
   - Gather the source material: every merged PR since the previous stable tag (`git log <last-tag>..origin/main --oneline` plus `gh pr list --state merged --base main --search "merged:>=<last-tag-date>"` (date-scoped, unpaginated result set), cross-checked against the `git log <last-tag>..origin/main` range). Read each PR description and verify coverage against the actual diff. Never draft from commit or PR titles alone — they routinely omit user-facing features (hotkeys, pickers, Settings toggles, menu actions) that only the PR body or diff reveals.
   - Write the entry for users, not for commit logs: explain concisely what each feature is and how to use it — name the actual shortcut (⌃⌥⌘N), the Settings section, or the menu action involved, so the changelog doubles as a usage guide. Match the voice of the existing entries. Skip CI, release-pipeline, and internal refactor work, or collapse it into one honest "Behind the scenes" line.
   - Format: prepend the entry as the first element of `releases` in `changelog.json` at the repository root with the chosen version and today's UTC date. Use `new` for New features and `fixed` for Improvements and bug fixes. Each item is `{ "title": "...", "text": "...", "pr": <number> }`: give every named feature a short `title` lead-in (shown as an ink lead-in on the website and bolded in the GitHub release notes), omit `title` for small fixes, and omit `pr` only when no PR is associated. The same entry becomes the GitHub release notes, so write `text` that stands alone without the PR reference and attach the PR number to every item that has one.
...
5. Start and safely continue the official release unless the user opted out.
   ...
   - Before changing the draft to a published immutable release, require every temporary `*-app-notary.zip` and `*-dmg-notary.dmg` handoff asset to be deleted. A cleanup failure must leave the verified final artifacts in a resumable private draft rather than publishing temporary assets that can no longer be removed.
...
6. Verify the production distribution chain after notarization.
   - Confirm the stable GitHub release is public, non-prerelease, latest, contains the versioned DMG, versioned app ZIP, stable `Dayline.dmg`, and signed `appcast.xml`, and contains no temporary notarization handoff assets.
